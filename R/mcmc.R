                            ...
                            for_ggmcmc = TRUE)
{
  ...
  {
    output = data.frame(matrix(0,max(all_output_rows),output_cols+2))
    ...
    {
      nParameters =
      output = pivot_longer(output, output_names, names_to = "Parameter", values_to = "value")
...
#' A trace plot of the MCMC output.
#'
#' @param tidy_mcmc_output MCMC output (from load_mcmc_output or otherwise).
#' @param variable The name of the column shown in the trace plot.
#' @param from (optional) The index of the first row to be plotted.
#' @param to (optional) The index of the final row to be plotted.
#' @param step (optional) If step=n, we only plot every n'th point.
#' @return A list containing the MCMC chains.
#' @export
trace_plot = function(mcmc_output)
